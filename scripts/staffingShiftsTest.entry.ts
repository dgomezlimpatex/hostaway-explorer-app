...
async function collaboratorRuleForwarding() {
  ...
  // Regla de Dani: la ficha es la única fuente de horas; la disponibilidad registrada
  // no genera capacidad. 0 h en ficha queda fuera de la previsión, da igual que tenga.
  const result = await readStaffingDataset(fixture(tables), 's', day, day, rules);
  assert.equal(result.workers.length, 0, '0-hour worker is excluded even with availability');
  assert.ok(result.issues.some(i => i.code === 'zero-hour-rule-excluded'), 'exclusion is visible in criteria');
  console.log('PASS 0-hour workers excluded from forecast even with availability');
}
