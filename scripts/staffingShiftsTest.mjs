...
const out = mkdtempSync(join(tmpdir(), 'staffing-shifts-test-'));
try {
  const outfile = join(out, 'test.mjs');
  const result = await build({ entryPoints: ['scripts/staffingShiftsTest.entry.ts'], outfile, bundle: true, platform: 'node', format: 'esm', logLevel: 'silent', metafile: true,
  ...
  globalThis.fetch = async () => { throw new Error('Network forbidden in staffing shifts tests'); };
  try { await (await import(pathToFileURL(outfile).href)).run(process.argv[2]); }
  finally { globalThis.fetch = originalFetch; }
  console.log('staffing-shifts-tests: OK (offline adapter + real readClient spec validation)');
