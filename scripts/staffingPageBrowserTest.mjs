...
try {
  ...
  await expect(page.getByLabel('Horizonte de previsión')).toHaveValue(defaultMonths);
  page.once('dialog', dialog => dialog.accept());
  await page.getByLabel('Horizonte de previsión').selectOption('6');
  await expect(page.getByLabel('Resumen semanal de sede')).toHaveCount(0);
  await page.getByLabel('Mes inicial de previsión').fill('2026-08');
  await expect(page.getByLabel('Mes inicial de previsión')).toHaveValue('2026-08');
  await page.getByLabel('Mes inicial de previsión').fill('2026-10');
  await expect(page.getByLabel('Mes inicial de previsión')).toHaveValue('2026-10');
  await page.getByRole('button', { name: 'Consultar datos de la sede' }).click();
  await page.evaluate(() => window.finish());
