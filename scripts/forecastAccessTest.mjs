...
try {
  for (const mode of ['denied', 'missing-sede', 'allowed', 'other-user']) {
    ...
    else { assert.equal(query.queryKey[1], mode === 'other-user' ? 'u2' : 'u1'); assert.equal(query.queryKey[2], 's'); assert.equal(query.queryKey[3], '2026-09-28'); assert.equal(query.queryKey[4], '2026-11-01'); assert.match(query.queryKey[5], /2026-09-21/); assert.match(html, /Navegación del previsor/); assert.ok(!/href="\/(workers|calendar|planning-settings)/.test(html)); }
