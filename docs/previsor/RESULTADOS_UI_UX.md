...
- Equipo tiene filtros, búsqueda, orden, exclusiones separadas y estado visible en móvil. El perfil separa balance mensual, semana, disponibilidad y tareas.
...
- Informes comparte las mismas filas filtradas con el CSV y añade totales que conservan valores desconocidos. Ausencias y mantenimiento muestran su efecto, sin inventar ajustes de ausencias parciales.
- Reglas y datos expone fórmulas, prioridades y exclusiones. Las incidencias se agrupan por causa y dirigen al registro; los identificadores técnicos quedan en un segundo nivel.
...
| `npm run test:forecast:ui-ux` | 10 pruebas de presentación y 13 comprobaciones de navegador correctas |
...
| A17 | Refuerzo 15,5 h, entrada inválida, intervalo semanal, restauración conservando reserva; sin escrituras |
...
| A21 | Archivos de horas/ausencias descargados, abiertos por la prueba y contrastados |
| A22 | Capturas y verificaciones responsive, menú móvil y filtros |
...
Archivos de producto: `ForecastWorkspaceView`, `ForecastUi`, `ForecastOverview`, `ForecastOperations`, `ForecastDetails`, `ForecastReports`, `forecastPresentation`, `forecastReportData`, `forecast.css`, `forecastContract`, `forecastModel`, `forecastReader`, `forecastExport`, todos dentro de `src/features/staffing`. Resto: script de npm, pruebas y documentación.
