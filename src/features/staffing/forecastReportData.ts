...
export function buildReport(dataset: ForecastDataset, model: ForecastModel, month: string, type: ForecastReportType, query = '', status = '') {
  ...
  return { headings, rows, totals, scope, evaluated: ledgers.filter(l => l.status !== 'No verificable').length, unknown: ledgers.filter(l => l.status === 'No verificable').length };
}
