...
export function StaffingComparison({ baseline, current, changed, uncertain, days, onOpenScenario }: Props) {
  ...
  return <section className={`${panelClass} overflow-hidden`} aria-label="Comparación de escenarios">
    ...
    {!baseline || !current ? <p className="mt-5 rounded-lg bg-[#faf9fc] p-4 text-sm text-[#716a7d]">Selecciona una semana con datos para comparar alternativas.</p> : <div className="mt-5 grid gap-3 sm:grid-cols-3">
      <div className="rounded-lg border border-[#ece8f3] bg-[#fcfbfe] p-4"><p className="text-xs font-semibold uppercase tracking-wide text-[#817a8c]">Trabajo ya registrado</p><p className="mt-2 text-lg font-bold tabular-nums text-[#201936]">{hours(baseline.knownMinutes)} <span className="text-sm font-normal text-[#aaa3b3]">→</span> {hours(current.knownMinutes)}</p><p className="mt-1 text-xs text-[#716a7d]">Horas de trabajo registradas</p></div>
      <div className="rounded-lg border border-[#ece8f3] bg-[#fcfbfe] p-4"><p className="text-xs font-semibold uppercase tracking-wide text-[#817a8c]">Horas del equipo</p><p className="mt-2 text-lg font-bold tabular-nums text-[#201936]">{hours(baseline.capacityMinutes)} <span className="text-sm font-normal text-[#aaa3b3]">→</span> {hours(current.capacityMinutes)}</p><p className="mt-1 text-xs text-[#716a7d]">No son horas libres de verdad</p></div>
      <div className="rounded-lg border border-[#d8eee9] bg-[#f7fcfb] p-4"><p className="text-xs font-semibold uppercase tracking-wide text-[#19766d]">Trabajo sin repartir</p><p className="mt-2 text-lg font-bold tabular-nums text-[#19766d]">{hours(baseUncovered)} <span className="text-sm font-normal text-[#aaa3b3]">→</span> {hours(currentUncovered)}</p><p className="mt-1 text-xs text-[#557d78]">{changed ? `${hours(improvement)} reducidas con la simulación` : 'No incluye estimaciones'}</p></div>
    </div>}
    <div className="mt-4 grid gap-2 border-t border-[#eeeaf4] pt-4 text-xs text-[#716a7d] sm:grid-cols-3"><span><strong className="block text-[#201936]">{affectedDays || '—'}</strong>Días con trabajo sin repartir</span><span><strong className="block text-[#201936]">{hours(assignedMinutes)}</strong>Trabajo asignado en la simulación</span><span><strong className="block text-[#201936]">{baseline && current && baseline.cost !== null && current.cost !== null ? `${money(baseline.cost)} → ${money(current.cost)}` : 'No disponible'}</strong>Coste solo con datos suficientes</span></div>
    <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-[#eeeaf4] pt-3 text-xs text-[#817a8c]"><span className="inline-flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#19766d]" />{baseline ? `Semana desde ${shortDate(baseline.week)}` : 'Sin semana seleccionada'}</span><span className="inline-flex items-center gap-1.5"><CircleHelp className="h-3.5 w-3.5" />{uncertain ? 'Faltan datos: revisa las fuentes.' : 'Resultado calculado con las fuentes disponibles.'}</span></div>
  </section>;
}
