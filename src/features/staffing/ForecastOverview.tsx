...
function TeamAttention({ model, to, month, control }: Pick<ScreenProps, 'model' | 'to'> & { month: string; control?: ReactNode }) {
  const team = teamSummary(model, month);
  if (!model.months.some(m => m.key === month)) return <div className="sf-metric sf-team-attention">{control}<span>Objetivos de {monthLabel(month)} · sin calcular</span><Link to={to('team', { focusMonth: month })}>Consultar este mes</Link></div>;
  const figure = (count: number, label: string, filter: string) => {
    const contents = <><strong>{count}</strong><span>{label}</span></>;
    return count ? <Link className="sf-attention-primary" to={to('team', { focusMonth: month, filter })}>{contents}</Link> : <div className="sf-attention-primary">{contents}</div>;
  };
  return <div className={'sf-metric sf-team-attention ' + (team.unknown.length ? 'sf-attention' : '')}>
    <div className="sf-objective-heading"><span>Objetivos de {monthLabel(month)}</span>{control}</div>
    <div className="sf-attention-stats">{figure(team.risk.length, 'con horas por completar', 'risk')}{figure(team.unknown.length, 'pendientes de evaluar', 'unknown')}</div>
    {team.risk.length > 0 && <small>{hours(sumKnown(team.risk, l => l.missing))} por completar según compromisos leídos</small>}
  </div>;
}
...
export function ForecastHome(props: ScreenProps) {
  ...
  return <>
    ...
    <div className="sf-metrics sf-three"><TeamAttention {...props} month={scope.from.slice(0, 7)} /><Metric label="Tareas sin asignar" value={tasks.length} note={scope.label} to={tasks.length ? to('shifts', { ...scopeQuery(scope), filter: 'pending' }) : undefined} /><Metric label="Días próximos por resolver" value={review.upcoming.length} note={review.past.length + ' días anteriores por revisar por separado'} to={review.upcoming.length ? to('shifts', { ...scopeQuery(scope), filter: 'uncovered' }) : undefined} /></div>
...
export function ForecastSummary(props: ScreenProps & { onSimulate: () => void }) {
  ...
  const monthlyTeam = (month: string) => { const team = teamSummary(model, month); return <div className="sf-month-team">{team.unknown.length ? <Link to={to('team', { focusMonth: month, filter: 'unknown' })}>{countLabel(team.unknown.length, 'persona')} por evaluar</Link> : <span>Ninguna persona pendiente de evaluar</span>}{team.risk.length ? <Link to={to('team', { focusMonth: month, filter: 'risk' })}>{countLabel(team.risk.length, 'persona')} con horas pendientes</Link> : <span>Sin horas pendientes identificadas</span>}</div>; };
  return <>
    ...
    {weekly && scope.from.slice(0, 7) !== scope.to.slice(0, 7) && <div className="sf-inline-links"><span>Semana entre dos meses:</span>{[scope.from.slice(0, 7), scope.to.slice(0, 7)].map(m => <Link key={m} to={to('team', { focusMonth: m })}>Objetivos de {monthLabel(m)}</Link>)}</div>}
    <div className="sf-metrics sf-four sf-forecast-metrics"><TeamAttention {...props} month={focus} control={<label>Mes del objetivo<select value={focus} onChange={e => change({ focusMonth: e.target.value })}>{[...new Set([...model.months.map(m => m.key), focus, ...(weekly ? [scope.from.slice(0, 7), scope.to.slice(0, 7)] : [])])].sort().map(m => <option key={m} value={m}>{monthLabel(m)}{model.months.some(row => row.key === m) ? '' : ' · sin calcular'}</option>)}</select></label>} /><Metric label={demand.unknown.length ? 'Subtotal turístico conocido' : 'Trabajo turístico conocido'} value={hours(demand.known)} note={demand.unknown.length ? <Link to={to('shifts', { ...scopeQuery(scope), filter: 'unknown-duration' })}>{countLabel(demand.unknown.length, 'tarea')} sin duración · total pendiente</Link> : 'Duración de las tareas registradas'} /><Metric label={demand.unknown.length ? (hasReserve ? 'Subtotal con reserva' : 'Reserva sobre subtotal conocido') : hasReserve ? weekly ? 'Total semanal con reserva' : 'Total con reserva' : 'Reserva adicional estimada'} value={hours(hasReserve ? demand.known * 1.2 : demand.reserve)} note={(demand.unknown.length ? 'Subtotal; faltan duraciones. ' : '') + (hasReserve ? 'Incluye ' + hours(demand.reserve) + ' de reserva · cifras redondeadas' : '+20 % sobre turismo · no activada')} /><Metric label="Tareas por revisar" value={reviewTasks.length} note="Registros por corregir, datos insuficientes o sin propuesta" to={reviewTasks.length ? to('shifts', { ...scopeQuery(scope), filter: 'uncovered' }) : undefined} /></div>
    ...
    <Panel title={weekly ? 'Detalle de los siete días' : 'Trabajo por resolver desde hoy'}>
      ...
      <div className="sf-day-list">{weekly ? days.map(d => dayRow(d)) : attention.upcoming.slice(0, 3).map(d => dayRow(d, d.reasons))}</div>
      ...
      {!weekly && attention.upcoming.length > 3 && <div className="sf-panel-actions"><Link className="sf-button" to={to('shifts', { ...scopeQuery(scope), filter: 'uncovered' })}>Ver todo el trabajo por revisar · {attention.upcoming.length} días próximos</Link></div>}
      ...
      <div className="sf-panel-actions">{tasks.some(t => !t.workerId) && <Link className="sf-button" to={to('shifts', { ...scopeQuery(scope), filter: 'pending' })}>{countLabel(tasks.filter(t => !t.workerId).length, 'tarea')} sin asignar · ver candidatos</Link>}{teamSummary(model, focus).unknown.length > 0 && <Link to={to('team', { filter: 'unknown', focusMonth: focus })}>{countLabel(teamSummary(model, focus).unknown.length, 'persona')} por verificar en {monthLabel(focus)}</Link>}</div>
    </Panel>
  </>;
}
...
export function SimulationResult(props: ScreenProps & { scope: ViewScope; hoursPerWeek: number }) {
  ...
  return <Panel title="Comparación del refuerzo con el equipo actual">
    <p className={'sf-simulation-outcome ' + (!delta.improved.length ? 'sf-no-gain' : '')}>{delta.improved.length ? countLabel(delta.improved.length, 'tarea') + (delta.improved.length === 1 ? ' adicional; ' : ' adicionales; ') + hours(used) + ' de ' + hours(hoursPerWeek * 60 * weekCount) + ' utilizadas; quedan ' + delta.remaining.length + ' sin propuesta viable' : 'El refuerzo no aporta encajes adicionales con los datos disponibles.'}</p>
    <p className="sf-caption">Sin propuesta viable: {delta.before.length} antes → {delta.remaining.length} después.</p>
    <details className="sf-detail-block"><summary>Desglose del resultado y reconciliación</summary><div className="sf-metrics sf-four"><Metric label="Sin propuesta viable antes" value={delta.before.length} note={hours(demandSummary(delta.before).known)} /><Metric label="Sin propuesta viable después" value={delta.remaining.length} note={hours(demandSummary(delta.remaining).known)} /><Metric label="Horas del refuerzo utilizadas" value={hours(used)} note={countLabel(proposals.length, 'tarea') + ' con encaje hipotético'} /><Metric label="Límite de horas sin utilizar" value={hours(Math.max(0, hoursPerWeek * 60 * weekCount - used))} note={hours(hoursPerWeek * 60) + ' × ' + countLabel(weekCount, 'semana') + '; límite, sin reparto diario'} /></div>
    <p className="sf-caption">{delta.before.length} pendientes − {delta.reinforcement.length} resueltas por refuerzo − {delta.team.length} por reorganización del equipo + {delta.lost.length} que pierden encaje = {delta.remaining.length} pendientes. Se conservan {countLabel(team.length, 'propuesta')} del equipo actual. Ninguna se guarda.</p></details>
    <p className="sf-caption sf-simulation-records">{delta.conflicts.length > 0 ? <Link to={props.to('shifts', { ...scopeQuery(scope), filter: 'record-conflicts' })}>{countLabel(delta.conflicts.length, 'registro')} por corregir</Link> : 'Sin conflictos del registro identificados.'}{delta.recordsWithFit.length > 0 && <> · <Link to={props.to('shifts', { ...scopeQuery(scope), filter: 'record-with-fit' })}>{delta.recordsWithFit.length} ya tienen encaje</Link>; esos registros no requieren capacidad adicional según el cálculo, pero deben corregirse.</>}{delta.incomplete.length > 0 && <> · <Link to={props.to('shifts', { ...scopeQuery(scope), filter: 'insufficient' })}>{countLabel(delta.incomplete.length, 'tarea')} con datos insuficientes</Link>; no se evalúan como falta de capacidad.</>}</p>
