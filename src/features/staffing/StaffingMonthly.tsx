...
export function StaffingMonthly({ view, selectedMonth, selectedWeek, onMonth, onWeek, onOpenScenario, onOpenData, availableWeeks }: {
  ...
  return <section aria-label="Previsión mensual" className="space-y-4">
    ...
    <div role="tablist" aria-label="Meses de previsión" className="grid min-w-0 gap-3 sm:grid-cols-3">
      {view.months.map(item => <button key={item.month} type="button" role="tab" aria-selected={item.month === month.month} aria-label={`${item.label}: ${hours(item.knownMinutes)} horas de trabajo previsto; ${hours(item.knownMinutes + item.estimatedMinutes)} horas de trabajo posibles con hipótesis; equipo actual ${hours(item.capacityMinutes)} horas posibles${item.status === 'current-partial' ? ', mes en curso parcial' : ''}`} onClick={() => onMonth(item.month)} className={`min-w-0 rounded-lg border bg-white p-4 text-left transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#310984] ${item.month === month.month ? 'border-[#310984] bg-[#f3eff8] ring-1 ring-[#310984]' : 'border-stone-200'}`}>
        <span className="block text-xs font-bold uppercase tracking-[0.12em] text-[#310984]">{item.label}</span><strong className="mt-2 block text-2xl tabular-nums sm:text-3xl">{hours(item.knownMinutes)}</strong><span className="mt-1 block text-xs text-stone-600">Horas de trabajo previsto</span><span className="mt-2 block text-xs text-stone-600">{hours(item.knownMinutes + item.estimatedMinutes)} · Horas de trabajo posibles (hipótesis; la reserva cercana no se extiende automáticamente a todo el horizonte)</span><span className="mt-2 block text-xs font-medium text-stone-600">Equipo actual · {hours(item.capacityMinutes)} h posibles</span>
      </button>)}
    </div>
    <article aria-label={`Visión de conjunto de ${month.label}`} className="rounded-lg border border-stone-200 bg-white p-4 sm:p-6">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="min-w-0"><div className="flex flex-wrap items-baseline justify-between gap-2 text-sm"><span className="text-stone-600">Registrado</span><strong className="text-2xl tabular-nums">{hours(month.knownMinutes)}</strong></div><div className="mt-3 h-5 overflow-hidden rounded-full bg-stone-100" aria-label={`Banda de trabajo ${band(month)}`}><span className="block h-full bg-[#310984]" style={{ width: `${Math.min(100, month.knownMinutes / max * 100)}%` }}/><span className="block h-full -translate-y-full bg-[#cfc1df]" style={{ width: `${Math.min(100, (month.knownMinutes + month.estimatedMinutes) / max * 100 - month.knownMinutes / max * 100)}%`, backgroundImage: 'repeating-linear-gradient(135deg,#9376ba 0 2px,#e9e0f4 2px 5px)' }}/></div><div className="mt-2 flex flex-wrap justify-between gap-2 text-sm"><strong className="text-[#310984]">Banda de trabajo: {band(month)}</strong><span className="font-semibold text-emerald-700">Capacidad potencial: {hours(month.capacityMinutes)}</span></div></div>
