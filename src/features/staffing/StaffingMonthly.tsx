...
export function StaffingMonthly({ view, selectedMonth, selectedWeek, onMonth, onWeek, onOpenScenario, onOpenData, availableWeeks }: {
  ...
  return <section aria-label="Previsión mensual" className="space-y-4">
    ...
    <div role="tablist" aria-label="Meses de previsión" className="grid min-w-0 gap-3 sm:grid-cols-3">
      {view.months.map(item => <button key={item.month} type="button" role="tab" aria-selected={item.month === month.month} aria-label={`${item.label}: ${hours(item.knownMinutes)} horas de trabajo previsto; ${hours(item.knownMinutes + item.estimatedMinutes)} horas de trabajo posibles; equipo actual ${hours(item.capacityMinutes)} horas posibles${item.status === 'current-partial' ? ', mes en curso parcial' : ''}`} onClick={() => onMonth(item.month)} className={`min-w-0 rounded-lg border bg-white p-4 text-left transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#310984] ${item.month === month.month ? 'border-[#310984] bg-[#f3eff8] ring-1 ring-[#310984]' : 'border-stone-200'}`}>
        <span className="block text-xs font-bold uppercase tracking-[0.12em] text-[#310984]">{item.label}</span><strong className="mt-2 block text-2xl tabular-nums sm:text-3xl">{hours(item.knownMinutes)}</strong><span className="mt-1 block text-xs text-stone-600">Horas de trabajo previsto</span><span className="mt-2 block text-xs text-stone-600">{hours(item.knownMinutes + item.estimatedMinutes)} · Horas de trabajo posibles (hasta un 20 % de nuevas reservas)</span><span className="mt-2 block text-xs font-medium text-stone-600">Equipo actual · {hours(item.capacityMinutes)} h posibles</span>
      </button>)}
    </div>
