...
export function mapStaffingWorkers(input: WorkerInputs): StaffingWorker[] {
  ...
  return input.workers.filter(row => row.is_active === true).map(row => {
    ...
    for (const slot of slots) {
      ...
      if (!weekday(day)) { issues.push({ code: 'invalid-availability', message: 'Disponibilidad con día inválido; capacidad incompleta.' }); continue; }
    ...
    for (const absence of input.absences.filter(item => item.cleaner_id === id)) {
      ...
      if (!validDate(start) || !validDate(end) || end < start) { issues.push({ code: 'invalid-absence', message: 'Ausencia con fechas inválidas; capacidad incompleta.' }); continue; }
      ...
      if (!fullDay && !valid) issues.push({ code: 'invalid-absence', message: 'Ausencia parcial inválida: se bloquea el día como hipótesis prudente.' });
    ...
    for (const item of input.maintenance.filter(item => item.cleaner_id === id && item.is_active === true)) {
      ...
      if (type !== 'maintenance' && type !== 'unavailability') issues.push({ code: 'maintenance-type-assumption', message: 'Compromiso semanal sin schedule_type: bloqueado y descontado del contrato como hipótesis prudente.' });
      const startMinute = timeMinutes(item.start_time); const endMinute = timeMinutes(item.end_time);
      const valid = Number.isFinite(startMinute) && endMinute > startMinute;
      if (!valid) issues.push({ code: 'invalid-maintenance', message: 'Compromiso semanal con horas inválidas: día bloqueado como hipótesis prudente.' });
      for (const day of Array.isArray(item.days_of_week) ? item.days_of_week : []) {
        if (!weekday(day)) { issues.push({ code: 'invalid-maintenance', message: 'Compromiso semanal con día inválido.' }); continue; }
