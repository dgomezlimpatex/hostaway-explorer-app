...
export function StaffingTeam({ workers, days, week, onSimulate }: Props) {
  ...
  const table = (title: string, group: TeamRow[]) => <section aria-label={title} className="min-w-0">
    ...
    {group.length === 0 ? <p className="py-6 text-sm text-[#716a7d]">No hay personas que coincidan con el filtro.</p> : <div className="mt-2 overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><caption className="sr-only">{title} · semana {week || 'seleccionada'}</caption><thead className="text-xs text-[#817a8c]"><tr><th className="py-2 pr-3">Persona</th><th className="px-3 py-2 text-right">Horas disponibles</th><th className="px-3 py-2 text-right">Trabajo simulado</th><th className="px-3 py-2 text-right">Sin asignar</th><th className="px-3 py-2">Revisar</th></tr></thead><tbody>{group.map(row => {
      const progress = row.availableMinutes ? Math.min(100, row.assignedMinutes / row.availableMinutes * 100) : 0;
      const review = row.worker.canMove ? 'Movilidad' : row.worker.availability.length ? 'Horarios' : 'Centros';
      return <tr key={row.worker.id} className="border-t border-[#f0edf5] align-middle"><th className="py-3 pr-3 font-medium text-[#201936]"><div className="flex items-center gap-2"><span aria-hidden="true" className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#eee9f7] text-xs font-bold text-[#390b92]">{initials(row.worker.name)}</span><span className="min-w-0"><span className="block break-words">{row.worker.name}</span><span className="block text-xs font-normal text-[#817a8c]">{row.worker.engagement === 'collaborator' ? 'Colaborador habitual' : 'Plantilla'}</span></span></div></th><td className="px-3 py-3 text-right tabular-nums text-[#4f485b]">{hours(row.availableMinutes)}</td><td className="px-3 py-3 text-right tabular-nums text-[#4f485b]"><span className="flex items-center justify-end gap-2"><span aria-hidden="true" className="hidden h-1.5 w-16 overflow-hidden rounded-full bg-[#eeeaf4] sm:block"><span className="block h-full bg-[#390b92]" style={{ width: `${progress}%` }} /></span>{hours(row.assignedMinutes)}</span></td><td className={`px-3 py-3 text-right font-semibold tabular-nums ${row.unassignedMinutes > 0 ? 'text-[#a46c00]' : 'text-[#716a7d]'}`}>{hours(row.unassignedMinutes)}</td><td className="px-3 py-3 text-xs"><button type="button" onClick={onSimulate} className="inline-flex items-center gap-1 font-bold text-[#390b92]">{review} <ArrowRight className="inline h-3.5 w-3.5" /></button></td></tr>;
    })}</tbody></table></div>}
  </section>;

  return <section aria-label="Equipo de previsión" className={`${panelClass} space-y-5`}>
    <header className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[0.17em] text-[#390b92]">Equipo</p><h2 className="mt-1 text-xl font-bold tracking-tight text-[#201936]">Equipo y carga simulada</h2><p className="mt-1 text-sm text-[#716a7d]">Semana {week || 'seleccionada'} · disponibilidad registrada, no cuadrante real.</p></div><button type="button" className="inline-flex min-h-9 items-center gap-1 text-xs font-bold text-[#390b92]" onClick={onSimulate}>Editar escenario <ArrowRight className="h-3.5 w-3.5" /></button></header>
    ...
    <p className="rounded-lg border border-[#eee5c9] bg-[#fffaf0] p-3 text-xs leading-5 text-[#80621b]">Las horas sin asignar dependen del reparto simulado y de los datos disponibles. No significan automáticamente exceso de jornada ni necesidad de reducir personal.</p>
