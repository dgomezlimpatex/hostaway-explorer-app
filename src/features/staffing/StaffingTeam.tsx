...
interface TeamRow {
  ...
  /** Jornada comprometida del periodo: lo que la persona DEBE trabajar. */
  availableMinutes: number;
  /** Jornada comprometida que aún no tiene limpieza asignada. */
  unassignedMinutes: number;
  /** Tope operativo del periodo: jornada comprometida + 30 % como máximo. */
...
export function StaffingTeam({ workers, days, week, periodLabel, onSimulate }: Props) {
  ...
  const rows = useMemo<TeamRow[]>(() => workers.map(worker => {
    ...
    // «Sin asignar» no puede superar la jornada comprometida: es lo que falta por cubrir de lo que se paga.
    return { worker, assignedMinutes, availableMinutes, unassignedMinutes: Math.max(0, availableMinutes - assignedMinutes), maxAvailableMinutes };
