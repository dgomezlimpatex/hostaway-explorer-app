...
import { closeForecastDetail, focusMonth, fullDate, affectedRecords, horizonScope, scopedIssues, sourceLabel, viewScope, type ViewScope } from './forecastPresentation';
...
export function ForecastWorkspaceView(props: ViewProps) {
  ...
  return <div className="sf-workspace" onKeyDown={e => { if (e.key === 'Escape' && mobileMenu) { setMobileMenu(false); menuButton.current?.focus(); } }}>
    ...
    <main className="sf-main" id="forecast-content" tabIndex={-1}>
      ...
      <div id="forecast-actions" className={`sf-query-actions ${filters ? 'sf-open' : ''}`}>
      ...
      {screenProps && <>
        {scopedIssues(model, scope).length > 0 && screen !== 'settings' && <details className="sf-quality"><summary>{(() => { const affected = affectedRecords(dataset, scopedIssues(model, scope)); const summary = [affected.tasks.length && affected.tasks.length + ' tareas', affected.workers.length && affected.workers.length + ' personas'].filter(Boolean).join(' · '); return summary ? 'Revisar datos para evaluar: ' + summary : 'Revisar lecturas pendientes'; })() || 'Revisar lecturas pendientes'}</summary><Issues {...screenProps} scope={scope} compact /></details>}
