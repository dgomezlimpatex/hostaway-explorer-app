...
export function ForecastWorkspaceView(props: ViewProps) {
  ...
  return <div className="sf-workspace">
    ...
    <header className="sf-topbar"><Link to={to('home')} className="sf-logo"><span className="sf-logo-mark">L</span>LIMPATEX</Link><div className="sf-account">{props.sedeControl ?? <span><Building2 size={16} /> {sedeName}</span>}<span className="sf-readonly">Consulta y simulación</span></div><button className="sf-mobile-toggle" aria-expanded={mobileMenu} aria-controls="forecast-nav" onClick={() => setMobileMenu(!mobileMenu)}><Menu size={20} /> Menú</button></header>
