      ...
      <div className="mx-auto max-w-[1600px] space-y-5 p-4 sm:p-6 lg:p-8">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <Card className="rounded-[1.5rem] border-slate-200 shadow-sm">
              ...
              <CardContent className="space-y-4">
                ...
                {logisticsEnabled && <>
                  ...
                  {result && <div className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-100 p-4"><p className="text-xs uppercase text-slate-500">Coste logístico</p><p className="mt-1 text-xl font-black">{money(result.logistics.totalCost)}</p></div><div className="rounded-2xl bg-slate-100 p-4"><p className="text-xs uppercase text-slate-500">Precio logístico</p><p className="mt-1 text-xl font-black">{money(result.logistics.salePrice)}</p></div><div className="rounded-2xl bg-slate-100 p-4"><p className="text-xs uppercase text-slate-500">Base sin densidad</p><p className="mt-1 text-xl font-black">{money(result.logistics.baseCost)}</p></div></div>}
                </>}
              </CardContent>
            </Card>
          ...
          <aside className="space-y-5 xl:sticky xl:top-4 xl:self-start">
            <Card className="rounded-[1.5rem] border-slate-200 shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2"><Calculator className="h-5 w-5 text-[#310984]" />Resultado y escenario mensual</CardTitle></CardHeader><CardContent>{calculation.error && <p className="rounded-xl bg-red-50 p-3 text-sm font-semibold text-red-800">{calculation.error}</p>}{result && <div className="space-y-3"><div className="grid grid-cols-2 gap-3"><div className="rounded-2xl bg-slate-100 p-4"><p className="text-xs uppercase text-slate-500">Coste / rotación</p><p className="mt-1 text-xl font-black">{money(result.rotation.totalCost)}</p></div><div className="rounded-2xl bg-[#eee9ff] p-4 text-[#310984]"><p className="text-xs uppercase">Venta / rotación</p><p className="mt-1 text-xl font-black">{money(result.rotation.totalRevenue)}</p></div></div><div className="rounded-2xl border border-slate-200 p-4"><div className="flex justify-between text-sm"><span>Contribución</span><strong>{money(result.rotation.contribution)}</strong></div><div className="mt-2 flex justify-between text-sm"><span>Margen</span><strong>{pct.format(result.rotation.marginPercentage)} %</strong></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full ${result.rotation.contribution >= 0 ? 'bg-emerald-500' : 'bg-red-500'}`} style={{ width: `${Math.max(0, Math.min(100, result.rotation.marginPercentage))}%` }} /></div></div><div className="rounded-2xl bg-[#210554] p-4 text-white"><p className="text-xs uppercase text-white/70">Escenario mensual</p><p className="mt-1 text-3xl font-black">{money(result.monthly.totalRevenue)}</p><p className="mt-1 text-xs text-white/70">Venta estimada con {monthlyRotations} rotación(es). Contribución: {money(result.monthly.contribution)}</p></div><p className="text-xs text-slate-500">El PDF comercial solo mostrará precios de venta y no contiene costes, margen ni datos internos.</p></div>}</CardContent></Card>

            <Card className="rounded-[1.5rem] border-slate-200 shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2"><History className="h-5 w-5 text-[#310984]" />Presupuestos guardados · Historial de versiones</CardTitle></CardHeader><CardContent className="space-y-2">{budgetsQuery.isLoading && <p className="text-sm text-slate-500">Cargando presupuestos…</p>}{budgetsQuery.error && <div className="rounded-xl bg-red-50 p-3 text-sm text-red-800">No se pudo cargar la bandeja. Revisa la migración del módulo.</div>}{!budgetsQuery.isLoading && !budgetsQuery.error && budgets.length === 0 && <p className="text-sm text-slate-500">Aún no hay presupuestos persistidos.</p>}{budgets.map((budget) => <button key={budget.id} type="button" onClick={() => loadBudget(budget)} className={`w-full rounded-2xl border p-3 text-left transition hover:border-[#8b75d1] ${selectedBudgetId === budget.id ? 'border-[#310984] bg-[#f1edff]' : 'border-slate-200 bg-white'}`}><div className="flex items-start justify-between gap-2"><div><p className="font-bold text-slate-900">{budget.quote_number}</p><p className="mt-1 text-xs text-slate-500">{budget.prospect_name || 'Sin cliente'}</p></div><Badge variant={statusVariant(budget.status)}>{statusLabels[budget.status]}</Badge></div><div className="mt-2 flex justify-between text-xs text-slate-500"><span>v{budget.current_version_number}</span><strong className="text-slate-700">{money(budget.monthly_revenue)} / mes</strong></div></button>)}</CardContent></Card>
