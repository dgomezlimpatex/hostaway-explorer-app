          ...
          <div className="flex flex-col space-y-3">
            ...
            <div className="space-y-2 md:space-y-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className={`inline-flex items-center px-2 py-1 rounded-full font-medium bg-blue-100 text-blue-800 ${isMobile ? 'text-xs' : 'text-xs'}`}>
                  {userRole?.charAt(0).toUpperCase()}{userRole?.slice(1)}
                </span>
                {isSupervisor() && <span className={`inline-flex items-center px-2 py-1 rounded-full font-medium bg-amber-100 text-amber-800 ${isMobile ? 'text-xs' : 'text-xs'}`}>
                    Solo lectura
                  </span>}
              </div>
            </div>
              ...
              <div className="flex flex-col gap-3 md:gap-4">
                ...
                <div className="w-full">
                  <div className="relative">
                    ...
                    <Input placeholder={isCleaner() ? "Buscar en mis tareas..." : "Buscar tareas por propiedad, dirección o limpiador..."} value={searchTerm} onChange={e => onSearchChange(e.target.value)} className={`pl-10 ${isMobile ? 'h-12 text-base' : ''}`} />
                  </div>
                </div>
