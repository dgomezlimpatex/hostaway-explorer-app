...
export const PlanningWeeklyWorkloadPanel = ({ rows, startDate, endDate, isLoading, isError }: PlanningWeeklyWorkloadPanelProps) => (
  <Card className="border-line bg-white shadow-sm" data-planning-weekly-workload>
    <CardHeader className="pb-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <CardTitle className="flex items-center gap-2 text-base text-ink">
            <Users className="h-4 w-4 text-brand" /> Carga semanal del equipo
          </CardTitle>
        ...
        <span className="text-xs text-ink-3">La casa grande se reparte entre sus personas</span>
      </div>
    </CardHeader>
    <CardContent className="space-y-2">
      {isLoading && <p className="text-sm text-ink-3">Cargando horas semanales…</p>}
      ...
      {!isLoading && !isError && rows.length === 0 && <p className="text-sm text-ink-3">No hay personal activo para mostrar.</p>}
      {!isLoading && !isError && rows.map((row) => {
        ...
        return (
          <div key={row.cleanerId} className="rounded-md border border-line px-3 py-2.5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-ink">{row.cleanerName}</p>
                <p className="text-xs text-ink-3">{row.assignedTaskCount} servicio{row.assignedTaskCount === 1 ? '' : 's'} asignado{row.assignedTaskCount === 1 ? '' : 's'}</p>
              </div>
            ...
            <div className="mt-2 flex items-center gap-2 text-sm text-ink">
              ...
              <span className="text-ink-3">/ {hasContract ? formatHours(row.contractHours) : 'sin horas de contrato'}</span>
            ...
            {hasContract && (
              <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-line-soft" role="progressbar" aria-label={`Horas asignadas de ${row.cleanerName}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(percentage)}>
                <div className={`h-full ${row.status === 'overtime' ? 'bg-red-500' : row.status === 'near-limit' ? 'bg-amber-400' : 'bg-brand'}`} style={{ width: `${percentage}%` }} />
              </div>
            )}
