...
export function PlanningDayNavigation({date,disabled,onChange}:{date:Date;disabled?:boolean;onChange:(date:Date)=>void}) {
  return <nav aria-label="Cambiar día del planning" className="mb-4 flex flex-wrap items-center gap-2 rounded-2xl border border-[#310984]/10 bg-white p-3">
    ...
    <input aria-label="Día del planning" type="date" value={formatMadridDate(date)} disabled={disabled}
      className="h-11 min-w-0 rounded-lg border border-[#310984]/15 px-3 text-base"
    ...
    <p className="text-xs text-[#6b627a]">Cambiar de día no guarda el reparto.</p>
  </nav>;
}
