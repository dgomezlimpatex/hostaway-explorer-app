...
export const IssuesSection: React.FC<IssuesSectionProps> = ({
  ...
  const issueTypes = [
    ...
    { value: 'missing', label: 'Artículo faltante' },
    { value: 'maintenance', label: 'Mantenimiento' },
    { value: 'cleanliness', label: 'Problema de limpieza' },
    ...
    <div className="space-y-6">
                  ...
                  {issue.media_urls.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {issue.media_urls.map((url, index) => (
                        <div key={index} className="w-20 h-20 bg-gray-100 rounded border overflow-hidden">
                          <img
                            src={url}
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select
                value={newIssue.type}
                onValueChange={(value) => setNewIssue(prev => ({ ...prev, type: value as Issue['type'] }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setNewIssue(prev => ({ ...prev, severity: value as Issue['severity'] }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <div>
            ...
            <MediaCapture
              onMediaCaptured={handleNewIssueMediaAdded}
              reportId={reportId}
              checklistItemId={`issue-${Date.now()}`}
              existingMedia={newIssue.media_urls}
            />
          </div>
      ...
      {issues.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          ...
          <p>No se han reportado incidencias</p>
          <p className="text-sm">Las incidencias ayudan a mantener la calidad del servicio</p>
        </div>
      )}
    </div>
  );
