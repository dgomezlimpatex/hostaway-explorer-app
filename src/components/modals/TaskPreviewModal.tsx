...
export const TaskPreviewModal: React.FC<TaskPreviewModalProps> = ({
  ...
  const renderActionButtons = () => {
    ...
    if (isCleaner && task.cleanerId) {
      if (hasReport) {
        buttons.push(
          <Button key="view-report" onClick={() => { onViewReport?.(task); onOpenChange(false); }} className="flex-1" disabled={!isTaskFromToday}>
        ...
        buttons.push(
          <Button key="create-report" onClick={() => { onCreateReport?.(task); onOpenChange(false); }} className="flex-1" disabled={!isTaskFromToday}>
    ...
    if (canAssignCleaner && !task.cleanerId && !isCleaner) {
      buttons.push(
  ...
  const getModalClasses = () => {
    if (isMobile) return "w-full max-w-full h-full max-h-full m-0 rounded-none";
    return "max-w-2xl";
  };
        ...
        <div className={`${isMobile ? 'flex-1 overflow-y-auto' : ''} space-y-4 pt-2`}>
          ...
          <Card className="bg-gradient-to-br from-card to-card/50 border-0 shadow-lg">
            <CardContent className="p-4 space-y-3">
              {/* Dirección - clickable to Google Maps */}
              <a
                href={googleMapsUrl}
                target="_blank"
              ...
              {/* Asignado a - only for non-cleaners */}
              {!isCleaner && task.cleaner && (
          ...
          {/* Notas del piso - from property, shown for everyone */}
          {property?.notas && (
            <Card className="bg-gradient-to-br from-card to-card/50 border-0 shadow-lg">
          ...
          {task.notes && (
            <Card className="bg-gradient-to-br from-card to-card/50 border-0 shadow-lg">
              <CardContent className="p-4">
                ...
                <p className="text-sm text-foreground whitespace-pre-wrap bg-muted/30 p-3 rounded-lg border border-border/50">
                  {task.notes}
                </p>
              </CardContent>
            </Card>
          ...
          {/* Property details - only for non-cleaners */}
          {!isCleaner && property && (
            <Card>
              <CardContent className="p-4 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {(property.numeroCamasPequenas > 0) && (
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="font-medium">{property.numeroCamasPequenas}</p>
                  ...
                  {(property.numeroCamasSuite > 0) && (
                    <div className="flex items-center space-x-3">
                      <Bed className="h-4 w-4 text-muted-foreground" />
                      <div>
                        <p className="font-medium">{property.numeroCamasSuite}</p>
                  ...
                  {(property.numeroSofasCama > 0) && (
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="font-medium">{property.numeroSofasCama}</p>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">{formatTime(task.checkIn)} / {formatTime(task.checkOut)}</p>
                  </div>
                </div>
                {property.notas && (
                  <div>
                    <p className="font-medium">Notas especiales</p>
                    <p className="text-sm text-muted-foreground bg-muted p-3 rounded-md mt-1">{property.notas}</p>
                  </div>
                )}
              </CardContent>
            </Card>
          ...
          {/* Service info - only for non-cleaners */}
          {!isCleaner && (
            <Card>
              <CardContent className="p-4 space-y-3">
