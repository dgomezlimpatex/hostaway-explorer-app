    ...
    {properties.map(property => {
      ...
      return (
        <button key={property.id} type="button" aria-pressed={selected} onClick={() => onSelect(property)} className={cn(
          'flex min-h-[68px] w-full items-start gap-3 rounded-xl border p-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#310984] focus-visible:ring-offset-2',
          selected ? 'border-[#310984] bg-violet-50/60 ring-1 ring-[#310984]/20' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50',
        )}>
          ...
          <span className="min-w-0 flex-1">
            <span className="block break-words text-sm font-semibold leading-snug text-slate-950">{property.nombre}</span>
          ...
          <Collapsible key={group.id} open={open} onOpenChange={value => setExpanded(current => ({ ...current, [group.id]: value }))} className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            <CollapsibleTrigger asChild>
              <button type="button" className={cn(
                ...
                <span className="min-w-0 flex-1">
                  <span className="block break-words text-sm font-semibold text-slate-950">{group.name}</span>
