...
export function CrmDetailFrame({ title, subtitle, avatar, status, metricLabel, metricValue, actions, tabs, footer, children, variant = 'default' }: CrmDetailFrameProps) {
    ...
    <div className={isProperties ? 'flex min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:h-[calc(100dvh-180px)] lg:min-h-0' : 'flex min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:h-[calc(100dvh-270px)] lg:min-h-[640px]'}>
      <header className={isProperties ? 'shrink-0 border-b-4 border-[#310984] bg-slate-950 p-4 text-white sm:p-6' : 'shrink-0 border-b bg-gradient-to-br from-slate-950 via-slate-900 to-[#310984] p-4 text-white sm:p-6'}>
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            <span className={isProperties ? 'flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-white text-base font-semibold text-[#310984]' : 'flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-white text-base font-black text-[#310984]'}>{avatar}</span>
            <div className="min-w-0">
              <h2 className={isProperties ? 'break-words text-xl font-semibold tracking-tight sm:text-2xl' : 'break-words text-xl font-black sm:text-2xl'}>{title}</h2>
          ...
          <div className={isProperties ? 'shrink-0 rounded-xl border border-white/15 bg-white/5 px-3 py-2 xl:min-w-[150px]' : 'shrink-0 rounded-xl border border-white/10 bg-white/10 px-3 py-2 xl:min-w-[150px]'}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-slate-50 p-3 sm:p-5">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          <TabsList aria-label={`Secciones de ${title}`} style={{ gridTemplateColumns: `repeat(${tabs.length}, minmax(0, 1fr))` }} className="sticky top-0 z-10 grid h-auto w-full rounded-xl bg-white p-1 shadow-sm">
            {tabs.map(tab => <TabsTrigger key={tab.id} value={tab.id} className={isProperties ? 'min-h-11 min-w-0 rounded-lg px-1 py-2.5 text-xs font-medium sm:text-sm' : 'min-w-0 rounded-xl px-1 py-2.5 text-xs sm:text-sm'}>{tab.label}</TabsTrigger>)}
          </TabsList>
