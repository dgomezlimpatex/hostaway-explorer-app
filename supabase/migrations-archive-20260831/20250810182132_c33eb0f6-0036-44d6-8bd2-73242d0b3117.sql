-- Step 2: Create logistics schema objects and RLS
...
CREATE TABLE IF NOT EXISTS public.logistics_picklists (
  ...
  scheduled_date date,
  created_by uuid, -- profiles.id (no FK to auth.users)
  committed_by uuid,
...
CREATE POLICY IF NOT EXISTS "Admin and managers can manage picklists" ON public.logistics_picklists
  FOR ALL USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'manager'))
  WITH CHECK (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'manager'));

CREATE POLICY IF NOT EXISTS "Admin and managers can manage picklist items" ON public.logistics_picklist_items
  FOR ALL USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'manager'))
  WITH CHECK (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'manager'));

CREATE POLICY IF NOT EXISTS "Admin and managers can manage deliveries" ON public.logistics_deliveries
  FOR ALL USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'manager'))
  WITH CHECK (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'manager'));

CREATE POLICY IF NOT EXISTS "Admin and managers can manage delivery stops" ON public.logistics_delivery_stops
  FOR ALL USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'manager'))
  WITH CHECK (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'manager'));

CREATE POLICY IF NOT EXISTS "Admin and managers can manage delivery items" ON public.logistics_delivery_items
  FOR ALL USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'manager'))
  WITH CHECK (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'manager'));

-- Policies: Logistics role manage operational tables
CREATE POLICY IF NOT EXISTS "Logistics can manage picklists" ON public.logistics_picklists
  FOR ALL USING (has_role(auth.uid(), 'logistics'))
  WITH CHECK (has_role(auth.uid(), 'logistics'));

CREATE POLICY IF NOT EXISTS "Logistics can manage picklist items" ON public.logistics_picklist_items
  FOR ALL USING (has_role(auth.uid(), 'logistics'))
  WITH CHECK (has_role(auth.uid(), 'logistics'));

CREATE POLICY IF NOT EXISTS "Logistics can manage deliveries" ON public.logistics_deliveries
  FOR ALL USING (has_role(auth.uid(), 'logistics'))
  WITH CHECK (has_role(auth.uid(), 'logistics'));

CREATE POLICY IF NOT EXISTS "Logistics can manage delivery stops" ON public.logistics_delivery_stops
  FOR ALL USING (has_role(auth.uid(), 'logistics'))
  WITH CHECK (has_role(auth.uid(), 'logistics'));

CREATE POLICY IF NOT EXISTS "Logistics can manage delivery items" ON public.logistics_delivery_items
  FOR ALL USING (has_role(auth.uid(), 'logistics'))
  WITH CHECK (has_role(auth.uid(), 'logistics'));

-- Policies: Supervisors read-only
CREATE POLICY IF NOT EXISTS "Supervisors can view picklists" ON public.logistics_picklists
  FOR SELECT USING (has_role(auth.uid(), 'supervisor'));

CREATE POLICY IF NOT EXISTS "Supervisors can view picklist items" ON public.logistics_picklist_items
  FOR SELECT USING (has_role(auth.uid(), 'supervisor'));

CREATE POLICY IF NOT EXISTS "Supervisors can view deliveries" ON public.logistics_deliveries
  FOR SELECT USING (has_role(auth.uid(), 'supervisor'));

CREATE POLICY IF NOT EXISTS "Supervisors can view delivery stops" ON public.logistics_delivery_stops
  FOR SELECT USING (has_role(auth.uid(), 'supervisor'));

CREATE POLICY IF NOT EXISTS "Supervisors can view delivery items" ON public.logistics_delivery_items
  FOR SELECT USING (has_role(auth.uid(), 'supervisor'));
