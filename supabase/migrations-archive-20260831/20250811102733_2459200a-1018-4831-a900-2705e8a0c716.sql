...
BEGIN
    ...
    -- Insertar productos que faltan, usando ON CONFLICT para evitar duplicados
    INSERT INTO inventory_products (name, description, category_id, unit_of_measure, is_active, sort_order)
    VALUES 
        ('Sábanas Pequeñas', 'Sábanas para camas pequeñas/individuales', default_category_id, 'unidades', true, 2),
        ('Sábanas Suite', 'Sábanas para suite/cama principal', default_category_id, 'unidades', true, 3),
        ('Papel Higiénico', 'Rollos de papel higiénico', default_category_id, 'rollos', true, 8),
        ('Papel Cocina', 'Rollos de papel de cocina', default_category_id, 'rollos', true, 9),
        ('Kit Alimentario', 'Kit básico de alimentación (sal, azúcar, aceite, etc.)', default_category_id, 'kits', true, 10),
        ('Amenities Cocina', 'Amenities para cocina (detergente, bayetas, etc.)', default_category_id, 'kits', true, 11)
