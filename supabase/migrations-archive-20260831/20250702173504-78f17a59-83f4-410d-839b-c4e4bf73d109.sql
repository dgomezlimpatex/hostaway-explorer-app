-- Corregir definitivamente la función accept_invitation eliminando la ambigüedad de user_id
CREATE OR REPLACE FUNCTION public.accept_invitation(token uuid, input_user_id uuid)
 RETURNS app_role
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE auth.users.id = input_user_id 
    ...
    VALUES (input_user_id, invitation_role)
