-- Finalizar un reporte cambia tasks.status a completed, pero no modifica el
-- horario. Evita que esa transición aislada genere task_modified.
...
BEGIN
  ...
  -- TaskReportModal finaliza el reporte actualizando únicamente el estado.
  -- Ese cierre no es un cambio de hora y no requiere WhatsApp al trabajador.
  IF OLD.status IS DISTINCT FROM NEW.status
     AND NEW.status = 'completed'
     AND (old_operational - 'status') IS NOT DISTINCT FROM (new_operational - 'status') THEN
    RETURN NEW;
  END IF;
...
COMMENT ON FUNCTION public.enqueue_task_modified_notifications() IS
  'Crea task_modified ante cambios operativos; ignora metadatos técnicos y la transición aislada a completed al finalizar reportes.';
