-- Hotfix de seguridad posterior a la baja atómica:
-- valida rol/sede en cada SECURITY DEFINER y separa ciclos legados.

ALTER TABLE public.cleaners
  ADD COLUMN IF NOT EXISTS activation_cycle_id uuid NOT NULL DEFAULT gen_random_uuid();

CREATE OR REPLACE FUNCTION public.rotate_cleaner_activation_cycle()
RETURNS trigger
LANGUAGE plpgsql
...
DROP TRIGGER IF EXISTS cleaners_rotate_activation_cycle ON public.cleaners;
CREATE TRIGGER cleaners_rotate_activation_cycle
BEFORE UPDATE OF is_active ON public.cleaners
FOR EACH ROW
EXECUTE FUNCTION public.rotate_cleaner_activation_cycle();
...
BEGIN
  ...
  IF coalesce(auth.role(), '') <> 'service_role'
    ...
    RAISE EXCEPTION 'No autorizado para desactivar trabajadores de esta sede'
      USING ERRCODE = '42501';
  END IF;
    ...
    LOOP
      ...
      ELSE
        ...
        INSERT INTO public.notification_events (
          ...
          concat(
            'task_cancelled:', v_task.id::text, ':', _cleaner_id::text,
            ':legacy-deactivation:', v_cleaner.activation_cycle_id::text
          ),
          'pending'
        )
...
REVOKE ALL ON FUNCTION public.get_future_pending_tasks_for_cleaner(uuid) FROM PUBLIC, anon;
