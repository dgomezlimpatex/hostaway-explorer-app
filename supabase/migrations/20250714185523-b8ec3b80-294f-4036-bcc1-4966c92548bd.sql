-- Limpiar invitaciones expiradas existentes
UPDATE public.user_invitations 
SET status = 'expired' 
WHERE status = 'pending' AND expires_at < now();

-- Verificar función verify_invitation para mejorar la validación
...
AS $function$
    SELECT EXISTS (
        ...
        WHERE invitation_token::text = token
        AND email = $2
        AND status = 'pending'
        AND expires_at > now()
    )
...
-- Mejorar función accept_invitation para mejor manejo de errores
...
DECLARE
    ...
    user_name TEXT;
BEGIN
    -- Obtener datos de la invitación usando text en lugar de uuid
